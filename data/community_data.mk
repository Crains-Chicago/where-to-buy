...
suburb_modified.csv : final/suburb.csv places.csv suburb_school_index.csv suburb_crime_index.csv
	csvjoin -I -c "Place","Place","community","community" $^ |\
		csvcut -c "Place","Avg Commute Time","Diversity Index","crime","schools","FIPS" - |\
		sed -e "1s/Place/community/" -e "1s/Avg Commute Time/commute/" |\
		sed -e "1s/Diversity Index/diversity/" -e "1s/FIPS/fips/" > $@

chicago_modified.csv : final/chicago.csv chicago_school_index.csv chicago_crime_index.csv
	csvjoin -I -c "community" $^ |\
		csvcut -c "community","Average Commute","Diversity Index","crime","schools" - |\
